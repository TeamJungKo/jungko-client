...
const NotificationModal: React.FC<Props> = ({ nickname, open, onClose }) => {
  ...
  const deleteAllNotices = async () => {
    try {
      ...
      console.log('알림을 삭제하는 도중 오류가 발생했습니다: ', error);
    }
  };
  ...
  useEffect(() => {
    getNotices()
      .then((res) => {
        const noticesWithFormattedTime = res.data.keywordNotices.map(
          (notice: Notice) => {
      ...
      .catch((error) =>
        console.log('알림을 가져오는 도중 오류가 발생했습니다: ', error)
      );
  }, [noticeReload, open]);
  ...
  return (
    <div>
      ...
      >
        <DialogTitle
          sx={{ fontSize: '28px', fontFamily: 'Jua' }}
        >{`${nickname}님의 알림`}</DialogTitle>
